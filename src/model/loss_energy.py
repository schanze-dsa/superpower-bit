...
Composition (默认的线性组合形式，可以在外部被 loss_weights 覆盖)�?    Π = w_int * E_int
        + w_cn * E_cn
        + w_ct * E_ct
        + w_tie * E_tie
        - w_pre * W_pre
...
Public usage (typical):
    ...
    # (可�? 若需要监控边界残差，可构�?BoundaryPenalty 并传�?bcs�?    # 硬约束模式下其能量不再计入损失�?
    ...
    # 若使用自适应权重，可在外部调�?loss_weights.update_loss_weights / combine_loss
...
Weighted PINN:
    - You can multiply extra per-sample weights into components:
        ...
        # 若仍需对边界残差加权，可自行在 BoundaryPenalty 内处理；默认不计入损�?    - If you need to reweight volume points, see TotalEnergy.scale_volume_weights().
"""
...
@dataclass
class TotalConfig:
    # coefficients for each term (基础权重；如�?loss_weights，可作为 base_weights)
    ...
    # 参考应力尺度，用于�?E_sigma 归一化为无量纲（例如取材料屈服强�?MPa�?    sigma_ref: float = 1.0
...
class TotalEnergy:
    ...
    - energy(...) 负责计算各个分量能量/残差，并返回�?        Π_total, parts_dict, stats_dict
      其中 parts_dict 可直接交�?train/loss_weights.py 做自适应加权�?
    - update_multipliers(...) 只负责外�?ALM 乘子更新（接�?摩擦）�?    """
    ...
    def _ensure_weight_vars(self):
        """确保权重变量已创建；兼容�?checkpoint/对象缺少新字段的场景�?""
    ...
    def scale_volume_weights(self, factor: float):
        ...
        """
        # 注意：DFEM 版本里，体积分权重通常�?ElasticityEnergy 内部管理�?        if getattr(self.elasticity, "w_tf", None) is None:
            return
    ...
    def energy(self, u_fn, params=None, tape=None, stress_fn=None):
        ...
        """
        # 某些情况下（例如加载�?checkpoint 构造的对象）可能缺少新增的权重变量，这里兜底创建�?        self._ensure_weight_vars()
        if not self._built:
    ...
    def _compute_parts(self, u_fn, params, tape=None, stress_fn=None):
        ...
        if self.preload is not None:
            ...
            # 同时暴露两种键：
            # - stats["preload"]：供 Trainer 直接读取（例如打�?bolt_deltas�?            # - stats["pre_preload"]：兼�?staged preload 的历史逻辑
            stats.update(pstats)
        ...
        # 应力监督 / 平衡残差：需要应力头与弹性算子缓�?        w_sigma = float(getattr(self.cfg, "w_sigma", 0.0))
        ...
        if use_sigma or use_eq:
            ...
            # 恢复到单元级：根�?rev 映射回每个单元的 4 个节点并取均�?            sigma_nodes_full = tf.gather(sigma_pred_nodes, rev)
            ...
            # 归一化：让应力监督项无量纲，避免其量�?尺度压过势能�?            sigma_ref = tf.cast(getattr(self.cfg, "sigma_ref", 1.0), dtype)
            ...
            if use_sigma:
                ...
                # Von Mises 应力（预�?物理），便于日志中评估与屈服强度的比�?                def _von_mises(sig: tf.Tensor) -> tf.Tensor:
    ...
    def _energy_staged(self, u_fn, stages, root_params, tape=None, stress_fn=None):
        ...
        与先前的“望向最终态”做差不同，这里以增量势能形式逐步累加�?        Π_step,i = (E_int + E_cn + E_ct + E_tie)_i - ΔW_pre,i
        并为相邻阶段的开�?滑移跳变乘以载荷跳变添加耗散式惩罚，使不同加载顺�?        能够影响无数据训练，同时保留 ALM 乘子在阶段间的自然演化�?        """
        dtype = self.dtype
        keys = [
            "E_int",
            "E_cn",
        ...
        for idx, stage_params in enumerate(stage_seq):
            ...
            for key in keys:
                ...
                if key == "W_pre":
                    ...
                    totals[key] = totals[key] + cur  # 原始累加，便于观察能量水�?
...
if __name__ == "__main__":
    # 仅做 API 连通性检查；真正�?DFEM/接触细节在各子模块里�?    from dataclasses import dataclass
    import numpy as np
    ...
    class DummyAsm:
        def __init__(self):
            # 4 节点四面体：节点编号故意使用�?0 基，以测试映�?            self.nodes = {
                10: (0.0, 0.0, 0.0),
                11: (1.0, 0.0, 0.0),
                12: (0.0, 1.0, 0.0),
                13: (0.0, 0.0, 1.0),
